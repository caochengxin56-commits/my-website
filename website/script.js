...
const imagePool = [
  ...
  { src: "./图片/projects/medical-bed-1.jpg", fallback: "./图片/projects/medical-bed-1.jpg", position: "center center" },
  { src: "./图片/projects/medical-bed-2.jpg", fallback: "./图片/projects/medical-bed-2.jpg", position: "center center" },
  { src: "./图片/projects/medical-diagnosis-1.jpg", fallback: "./图片/projects/medical-diagnosis-1.jpg", position: "center center" },
  { src: "./图片/projects/medical-checkup-1.jpg", fallback: "./图片/projects/medical-checkup-1.jpg", position: "center center" },
  { src: "./图片/projects/medical-checkup-2.jpg", fallback: "./图片/projects/medical-checkup-2.jpg", position: "center center" },
  { src: "./图片/projects/medical-consultation-1.jpg", fallback: "./图片/projects/medical-consultation-1.jpg", position: "center 30%" },
];
...
function getCampaignImageMeta(profile, index) {
  if (profile.category.includes("心脏")) {
    return imagePool[2];
  }

  if (profile.category.includes("烧伤") || profile.category.includes("呼吸")) {
    return imagePool[3];
  }

  if (profile.category.includes("血液") || profile.category.includes("肿瘤")) {
    return imagePool[4];
  }

  if (profile.category.includes("神经") || profile.category.includes("眼科")) {
    return imagePool[5];
  }

  if (profile.category.includes("骨科") || profile.category.includes("修复") || profile.category.includes("普外")) {
    return imagePool[7];
  }

  if (profile.category.includes("肾脏")) {
    return imagePool[6];
  }
...
function createCampaign(profile, index) {
  const imageMeta = getCampaignImageMeta(profile, index);
...
const hopeStories = [
  ...
  {
    slug: "noor-kidney-school-return",
    name: "努尔",
    age: 7,
    city: "拉姆安拉",
    condition: "肾脏治疗",
  ...
  {
    slug: "halima-hydrocephalus-recovery",
    name: "哈立玛",
    age: 6,
    city: "代尔拜拉赫",
    condition: "脑积水",
  ...
  {
    slug: "adnan-spine-standing-again",
    name: "阿德南",
    age: 9,
    city: "希伯伦",
    condition: "脊柱固定手术",
...
const hopeStoryMap = new Map(hopeStories.map((story) => [story.slug, story]));
...
function buildHopeStoryHref(story) {
  return `./hope-story.html?slug=${encodeURIComponent(story.slug)}`;
}
...
function renderHopeStoryCard(story) {
  return `
    <article class="hope-story-card">
      <a class="hope-story-link" href="${buildHopeStoryHref(story)}">
        <img
          src="${story.image}"
...
function renderHomePage() {
  ...
  if (homeMenuButton && homeMenuPanel && homeMenuShell) {
    ...
    const closeHomeMenu = () => {
      ...
      homeMenuShell.classList.remove("is-open");
      homeMenuButton.setAttribute("aria-expanded", "false");
      homeMenuButton.setAttribute("aria-label", "打开导航菜单");
      homeMenuButton.classList.remove("is-open");
    };

    const openHomeMenu = () => {
      homeMenuPanel.hidden = false;
      homeMenuShell.classList.add("is-open");
      homeMenuButton.setAttribute("aria-expanded", "true");
      homeMenuButton.setAttribute("aria-label", "关闭导航菜单");
      homeMenuButton.classList.add("is-open");
    };
          ...
          .map(
            (story) => `
              <article class="hope-story-detail-card">
                <a class="hope-story-detail-link" href="${buildHopeStoryHref(story)}">
                  <img
                    src="${story.image}"
                    data-fallback="${story.fallbackImage}"
                    onerror="this.onerror=null;if(this.dataset.fallback){this.src=this.dataset.fallback;}"
                ...
                <div class="hope-story-detail-body">
                  ...
                  <a class="home-link-btn hope-story-detail-cta" href="${buildHopeStoryHref(story)}">${story.linkLabel}</a>
                </div>
              </article>
            `
          )
...
function renderHopeStoryPage() {
  const root = document.querySelector("#hopeStoryPage");
  ...
  const params = new URLSearchParams(window.location.search);
  const slug = params.get("slug");
  const story = (slug && hopeStoryMap.get(slug)) || hopeStories[0];

  document.title = `${story.name}的希望故事 | 慈愈计划`;

  root.innerHTML = `
    <section class="listing-hero">
      <span class="directory-kicker">Story Detail</span>
      <h2>${story.name}，${story.age}岁 · ${story.title}</h2>
      <p>${story.city} ｜ ${story.condition} ｜ ${story.category}</p>
    </section>

    <section class="hope-story-detail-card hope-story-detail-card-single">
      <img
        src="${story.image}"
        data-fallback="${story.fallbackImage}"
        onerror="this.onerror=null;if(this.dataset.fallback){this.src=this.dataset.fallback;}"
